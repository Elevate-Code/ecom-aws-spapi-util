...
export default class AwsSpApiReportsUtils {
  ...
  async getV2SettlementReportDataFlatFile(event) {
    ...
    try {
      ...
      console.log('scheduledReportsList.reports', scheduledReportsList?.reports.length)
      if (scheduledReportsList && scheduledReportsList?.reports && Array.isArray(scheduledReportsList.reports) && scheduledReportsList?.reports.length > 0) {
        console.log('===scheduledReportsList', scheduledReportsList?.reports)
        return this.createApiResponse(scheduledReportsList?.reports);
      } else {
  ...
  async createGetLedgerSummaryViewRequest(args) {
    try {
      if (!args) {
        let date = new Date();
      ...
      if(args.reportOptions) {
        params.reportOptions = args.reportOptions;
      }
      let createRequestResponse = await this.createReportRequest(params);
  ...
  async createCustomReportRequest(args) {
    try {
      let createRequestResponse = await this.createReportRequest(args);
  ...
  async downloadFile(args) {
    try {
      ...
      let res = await sellingPartner.callAPI({
        api_path: `/reports/2021-06-30/documents/${args.documentId}`,
        method: 'GET'
      });
  ...
  async downloadFileStream(args) {
    try {
      ...
      const download = await sellingPartner.download(res, {
        stream: true // Indicating that the response should be streamed
      });

      return this.createApiResponse(download.data); // Return the stream wrapped in createApiResponse
    } catch (error) {
      console.log('downloadFileStream.error', error);
      return this.createApiResponse(null, error); // Handle error and wrap in createApiResponse
    }
  }
