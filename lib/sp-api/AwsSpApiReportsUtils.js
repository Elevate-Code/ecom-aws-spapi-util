...
export default class  AwsSpApiReportsUtils {
  ...
  async getV2SettlementReportDataFlatFile(event) {
    ...
    try {
      let scheduledReportsList = await this.getReports({ report_type: 'GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE', marketplaceIdsList: this.configOptions.marketplaceIdsList,
        createdSince: event.createdSince, createdUntil: event.createdUntil });
