...
export default class AwsSPConnectDBMySqlUtils {
  ...
  async init() {
    ...
    let create_ALL_ORDERS_DATA_BY_ORDER_DATE_GENERAL_table_sql = "CREATE TABLE IF NOT EXISTS\n" +
      "  `ALL_ORDERS_DATA_BY_ORDER_DATE_GENERAL` (\n" +
      "    `id` int DEFAULT NULL,\n" +
      ...
      "    PRIMARY KEY (`amazon_order_id`, `market_place_id`)\n" +
  ...
  async #execSql(sql, values) {
    if (!dbConfig) {
      throw new Error('Initialize the class with proper dbConfig first');
    }
    const myConfig = this.dbConfig;
  ...
  async insertOrdersRecordsInBulk(orders, market_place_id) {
    ...
    // orders = [orders[0]]
    try {
      let sql = `REPLACE INTO ALL_ORDERS_DATA_BY_ORDER_DATE_GENERAL (
            amazon_order_id,
            merchant_order_id,
            purchase_date,
            last_updated_date,
  ...
  async saveFileData(args) {
    try {
      if (args.report_type == 'GET_FBA_REIMBURSEMENTS_DATA') {
        ...
        let insertOrdersDataResponse = await this.insertOrdersRecordsInBulk(args.fileData, args.market_place_id)
        ...
        // let insertV2SettlementResponse = await insertV2Settlement(args.fileData, args.market_place_id)
