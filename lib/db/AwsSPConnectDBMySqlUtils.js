...
export default class AwsSPConnectDBMySqlUtils {
  ...
  async insertMerchantListing(dataArr, market_place_id) {
    console.log('insertMerchantListing Received data:', dataArr[0]);

    try {
      // Construct the SQL query with the correct number of placeholders
      const placeholders = dataArr.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ');
      const sql = `
      REPLACE INTO GET_MERCHANT_LISTINGS_ALL_DATA (
        ...
        bid_for_featured_placement, add_delete, pending_quantity, fulfillment_channel, 
        merchant_shipping_group, status, minimum_order_quantity, sell_remainder, market_place_id 
      ) VALUES ${placeholders};
    `;

      const values = dataArr.reduce((acc, data) => {
        acc.push(
          data['item-name'],
      ...
      console.log('Values to be inserted:', values[0]);
      const response = await this.execSqlWithInit(sql, values);
      console.log('Insert response:', response);
    } catch (error) {
      console.error('Error in insertMerchantListing:', error);
      throw error;
    }
    console.log("END insertMerchantListing");
  }
