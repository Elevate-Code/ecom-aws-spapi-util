...
export default class AwsSPConnectDBMySqlUtils {
  ...
  async insertReportStatus(args) {
    ...
    let params = [];
    try {
      // Determine which fields and placeholders to use based on provided arguments
      let fields = 'report_type, status, report_id, report_file_id';
      let placeholders = '?, ?, ?, ?';
      params.push(args.report_type, args.status, args.report_id, args.report_file_id);

      // Check if marketplaceId is provided and should be included
      if (args.marketplaceId) {
        fields += ', market_place_id';
        placeholders += ', ?';
        params.push(args.marketplaceId);
      }

      // Check if created_at is provided and should be included
      if (args.created_at) {
        fields += ', created_at';
        placeholders += ', ?';
        params.push(args.created_at);  // Passing the Date object directly
      }

      // Construct the SQL query using the determined fields and placeholders
      sql = `INSERT INTO reports_status (${fields}) VALUES (${placeholders})`;

      console.log('insertReportStatus.sql', sql, params);
      let response = await this.execSqlWithInit(sql, params);
  ...
  async insertLedgerSummaryData(dataArr) {
    let now = new Date();
    let today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    let formattedToday = today.toISOString().split('T')[0];  // Converts Date object to YYYY-MM-DD format
    ...
    let checkResult = await this.execSqlWithInit(checkSql, [formattedToday]);
    if (checkResult.length > 0) {
      console.log('Sync already performed or is currently running for this date:', formattedToday);
    ...
    let insertSql = `INSERT INTO reports_status (report_type, status, modified_date, market_place_id)
      VALUES ('GET_LEDGER_SUMMARY_VIEW_DATA', 'RUNNING', NOW(), ?)`;
    await this.execSqlWithInit(insertSql, [market_place_id]);
    ...
    let values = dataArr.map(data => [
      ...
      formattedToday,  // Use the formatted UTC date for ReportRequestDate
    ]);
    ...
    try {
      ...
      await this.execSqlWithInit(updateSql, [formattedToday]);
      ...
      let failSql = `UPDATE reports_status SET status = 'FAILED' 
        WHERE report_type = 'GET_LEDGER_SUMMARY_VIEW_DATA' AND DATE(modified_date) = ?`;
      await this.execSqlWithInit(failSql, [formattedToday]);
      throw error;
    }
  }
  ...
  async saveFileData(args) {
    try {
      if (args.report_type == 'GET_FBA_REIMBURSEMENTS_DATA') {
        ...
        let insertLedgerSummaryResponse = await this.insertLedgerSummaryData(args.fileData, args.market_place_id)
      }
