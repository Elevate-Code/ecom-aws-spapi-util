...
export default class AwsSPConnectDBMySqlUtils {
  ...
  async init() {
    if (!AwsSPConnectDBMySqlUtils.pool) {
      AwsSPConnectDBMySqlUtils.pool = mysql.createPool({
        ...this.mySqlDbConfig,
        waitForConnections: true,
        connectionLimit: 10, // Set the connection limit as per your requirement
        queueLimit: 0
      });
    }
  ...
  async #execSql(sql, values) {
    ...
    if (!AwsSPConnectDBMySqlUtils.pool) {
      throw new Error('Run the execSqlWithInit method');
    }

    const pool = AwsSPConnectDBMySqlUtils.pool;
  ...
  async insertMerchantListing(dataArr, market_place_id) {
    ...
    try {
      // Truncate the table
      const truncateSql = 'TRUNCATE TABLE GET_MERCHANT_LISTINGS_ALL_DATA';
      await this.execSqlWithInit(truncateSql, []);

      // Prepare the REPLACE INTO statement
      let replaceSql = `REPLACE INTO GET_MERCHANT_LISTINGS_ALL_DATA 
      ...
      await this.execSqlWithInit(replaceSql, [values]);
      console.log('Transaction complete.');
    } catch (error) {
      console.error('insertMerchantListing.error', error);
      throw error;
    }
  }
